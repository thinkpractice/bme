...
WatchNetEndpoint::WatchNetEndpoint(const char* title, int protocol)
...
int32 WatchNetEndpoint::Receive(void *buffer, size_t size, int flags)
{
...
int32 WatchNetEndpoint::Receive(BNetBuffer &buffer, size_t size, int flags)
{
	int32 returnVal = BNetEndpoint::Receive(buffer, size, flags);
	if(returnVal > 0)
	{
...
int32 WatchNetEndpoint::Send(const void *buffer, size_t size, int flags)
{
